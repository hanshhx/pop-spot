...
export default function PlanningPage() {
  ...
  const sendVote = (placeId: string, voteType: 'LIKE' | 'FIRE') => {
    ...
    stompClientRef.current.publish({
      destination: `/app/plan/${roomId}/vote`,
  ...
  const addPlaceToMap = (place: SearchResult) => {
    ...
    stompClientRef.current.publish({
      ...
      body: JSON.stringify({ type: 'ADD', data: dataStr, sender: myInfo.name }),
    });
  };
  ...
  const removeMarker = (m: MarkerData) => {
    ...
    stompClientRef.current.publish({
      destination: `/app/plan/${roomId}/action`,
      body: JSON.stringify({ type: 'REMOVE', data: dataStr, sender: myInfo.name }),
    });
  };
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onConnect: () => {
        ...
        client.publish({
          destination: `/app/plan/${roomId}/action`,
          body: JSON.stringify({ type: 'JOIN', data: myDataStr, sender: myNickname }),
        });
        ...
        client.subscribe(`/topic/plan/${roomId}`, (message) => {
            ...
            setMarkers((prev) =>
              prev.map((m) =>
                m.id === action.placeId
                  ? {
                      ...m,
                      likeCount: action.voteType === 'LIKE' ? action.count : m.likeCount,
                      fireCount: action.voteType === 'FIRE' ? action.count : m.fireCount,
                    }
                  : m,
              ),
            );
          ...
          if (action.type === 'ADD') {
            ...
            setParticipants((prev) =>
              prev.some((p) => p.name === name) ? prev : [...prev, { name, color }],
            );
    ...
    <div className="h-[100dvh] flex flex-col md:flex-row bg-[#111] text-white overflow-hidden">
      ...
      <div className="flex-1 relative h-[45vh] md:h-full border-b md:border-b-0 md:border-r border-white/10 shrink-0">
        ...
        <div className="absolute top-3 left-3 md:top-4 md:left-4 z-10 px-2.5 py-1.5 md:px-3 md:py-1.5 bg-black/80 backdrop-blur rounded-full border border-white/10 flex items-center gap-1.5 md:gap-2 shadow-xl">
          <div
            className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isConnected ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}
          ></div>
      ...
      <div className="w-full md:w-[400px] flex flex-col bg-[#1a1a1a] h-[55vh] md:h-full">
        ...
        <div className="p-3 md:p-5 border-b border-white/10 bg-[#1a1a1a]">
          ...
          <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-1">
            {participants.map((p, i) => (
              <div
                key={i}
                className="flex flex-col items-center gap-1 min-w-[36px] md:min-w-[40px]"
              >
                <div
                  className={`w-8 h-8 md:w-10 md:h-10 rounded-full ${p.color} border-2 border-[#1a1a1a] flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-md`}
                >
        ...
        <div className="p-3 md:p-4 border-b border-white/5 bg-[#1f1f1f]">
          <form onSubmit={handleSearch} className="relative">
            ...
            <Search
              size={14}
              className="md:w-4 md:h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"
            />
            {isSearching && (
              <Loader2
                size={14}
                className="md:w-4 md:h-4 absolute right-3 top-1/2 -translate-y-1/2 text-lime-500 animate-spin"
              />
            )}
          </form>
          ...
          {searchResults.length > 0 && (
            <motion.div
              ...
              className="mt-2 md:mt-3 max-h-[120px] md:max-h-[150px] overflow-y-auto custom-scrollbar bg-[#111] rounded-lg md:rounded-xl border border-white/5"
            >
              {searchResults.map((place) => (
                <button
                  ...
                  className="flex w-full justify-between items-center p-2.5 md:p-3 hover:bg-white/5 cursor-pointer border-b border-white/5 last:border-0 group text-left focus-visible:outline-2 focus-visible:outline-lime-300"
                >
                  <span>
                  ...
                  <PlusCircle
                    size={14}
                    className="md:w-4 md:h-4 text-gray-500 group-hover:text-lime-500"
                  />
                </button>
              ))}
            </motion.div>
                    ...
                    >
                      <div className="flex justify-between items-center mb-2.5 md:mb-3">
                        <div className="flex items-center gap-2 md:gap-3">
                          <div className="w-6 h-6 rounded-full bg-lime-300/20 text-lime-300 flex items-center justify-center text-xs font-bold">
                            {idx + 1}
                          </div>
                          <div>
                      ...
                      <div className="flex gap-1.5 md:gap-2">
                        <button
                          onClick={() => sendVote(m.id, 'LIKE')}
                          className="flex-1 flex items-center justify-center gap-1 md:gap-1.5 py-1 md:py-1.5 rounded-md md:rounded-lg bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 transition-all active:scale-95"
                        >
                          <ThumbsUp size={12} className={m.likeCount > 0 ? 'fill-blue-400' : ''} />{' '}
                          <span className="text-xs font-bold">{m.likeCount}</span>
                        </button>
                        <button
                          onClick={() => sendVote(m.id, 'FIRE')}
                          className="flex-1 flex items-center justify-center gap-1 md:gap-1.5 py-1 md:py-1.5 rounded-md md:rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 transition-all active:scale-95"
                        >
                          <Flame size={12} className={m.fireCount > 0 ? 'fill-red-400' : ''} />{' '}
                          <span className="text-xs font-bold">{m.fireCount}</span>
                        </button>
                      </div>
                    </motion.div>
