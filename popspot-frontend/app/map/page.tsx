...
const DESCRIPTION =
  '서울 팝업스토어 위치를 지도 한 화면에서. 성수 · 홍대 · 강남 등 지역별 팝업의 위치와 마감일을 로그인 없이 무료로 확인.';
    ...
    <main className="min-h-screen bg-white text-gray-900 dark:bg-[#0a0a0a] dark:text-white">
      <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
        ...
        <h1 className="mb-3 text-2xl font-bold md:text-4xl">
          서울 팝업스토어 지도{live.length > 0 && ` · ${live.length}곳`}
        </h1>
        <p className="mb-8 max-w-2xl text-sm text-muted-foreground md:text-base">
          서울에서 진행 중인 팝업스토어 위치를 지도 한 화면에서 봅니다. 지도를 움직이면 그 지역의
          팝업이 표시되고, 핀을 누르면 영업 기간과 마감일을 확인할 수 있습니다. 로그인 없이 무료.
          {openToday > 0 && ` 오늘 문 여는 곳은 ${openToday}곳입니다.`}
        </p>
        ...
        {regionLinks.length > 0 && (
          <section className="mt-10 border-t border-gray-200 pt-6 dark:border-white/10">
            <h2 className="mb-3 text-sm font-bold md:text-base">지역으로 좁혀 보기</h2>
        ...
        >
          위시 등록하고 내 팝업 모아보기 <ArrowRight size={16} />
        </Link>
      </div>
    </main>
  );
