...
const COLLECTION_TABLE: PolicyTable = {
  ...
  rows: [
    ...
    // 저장하는 항목은 전부 방침에 적혀 있어야 하므로 자동 생성 항목에서 따로 떼어 명시한다.
    {
      cells: [
        '자동 생성 (방문 통계)',
        '익명 방문자 ID (브라우저가 생성한 무작위 값), 세션 ID (30분간 활동이 없으면 새 값으로 교체), 방문한 페이지 경로, 화면에서 한 행동의 종류 (팝업 상세 열기·지도 검색·공유·찜·예약/공식 링크 이동 중 하나 — 정해진 목록 외의 값은 저장하지 않습니다), 대상 팝업 식별자, 브라우저·기기 종류(User-Agent), 회원/게스트 구분, 유입 출처 도메인 (예: naver.com — 전체 주소와 검색어는 저장하지 않음), 유입 캠페인 식별자 (UTM), 방문 일시 — 접속 IP 와 회원 개인정보는 저장하지 않으며, 이 기록은 회원 계정과 연결하지 않습니다',
        '페이지 방문 시',
      ],
    },
    {
        ...
        <article className="prose prose-sm dark:prose-invert max-w-none space-y-10 leading-relaxed">
          ...
          <Section title="제2조 (개인정보의 이용 목적)">
            ...
            <Note>
              ...
              자체 익명 방문 로그 (랜덤 방문자 ID · 세션 ID · 페이지 경로 · 행동 종류 · 대상 팝업
              식별자 · 봇 식별용 브라우저 정보(User-Agent) · 회원/게스트 구분 · 유입 출처 도메인 ·
              유입 캠페인 식별자를 기록하며, IP와 회원 개인정보는 수집하지 않음) 를 사용합니다.
            </Note>
            <Note>
          ...
          <Section title="제3조 (개인정보의 보유 및 이용 기간)">
            <Ul
              items={[
                ...
                '익명 방문 로그 (방문자 ID · 세션 ID · 경로 · 행동 종류 · 대상 팝업 식별자 · User-Agent · 회원/게스트 구분 · 유입 출처 도메인 · 유입 캠페인 식별자) — 90일간 보관 후 매일 자동 삭제',
                '관리자 접속기록 (관리자 계정 식별자 · 수행한 작업 · 대상 종류와 내부 식별자 · 성공 여부 · 접속지 정보 · 시각) — 개인정보처리시스템 접속기록으로 1년간 보관 후 삭제. 접속지 정보는 정밀도를 낮춰(예: 119.194.113.0) 저장하며 온전한 IP 는 저장하지 않습니다',
          ...
          <Section title="제11조 (쿠키 및 분석 도구)">
            <p>
              ...
              같은 저장소에 기록합니다. 세션 ID는 탭을 닫거나 30분간 활동이 없으면 사라지는
              <code>sessionStorage</code>에 둡니다. 익명 방문자 ID는 90일이 지나면 새 값으로
              교체되며, 사용자가 브라우저 사이트 데이터를 지우면 즉시 삭제됩니다. 광고용 추적 쿠키는
              사용하지 않습니다. Vercel Web Analytics와 Speed Insights는 쿠키를 사용하지 않으며,
              개인을 직접 식별하지 않는 접속 통계와 페이지 성능 지표를 수집합니다. Sentry는 오류
              진단에 사용하며 개인정보 자동 전송 옵션을 비활성화합니다. 국외 사업자에 대한 내용은
              제6조가 함께 적용됩니다.
            </p>
            <Note>
