...
const COLLECTION_TABLE: PolicyTable = {
  ...
  rows: [
    ...
    // 저장하는 항목은 전부 방침에 적혀 있어야 하므로 자동 생성 항목에서 따로 떼어 명시한다.
    {
      cells: [
        '자동 생성 (방문 통계)',
        '익명 방문자 ID (브라우저가 생성한 무작위 값), 세션 ID (30분간 활동이 없으면 새 값으로 교체), 방문한 페이지 경로, 화면에서 한 행동의 종류 (페이지 열람·팝업 상세 열기·지도 검색 등 정해진 목록 중 하나), 대상 팝업 식별자, 브라우저·기기 종류(User-Agent), 회원/게스트 구분, 유입 출처 도메인 (예: naver.com — 전체 주소와 검색어는 저장하지 않음), 유입 캠페인 식별자 (UTM), 방문 일시 — 접속 IP 와 회원 개인정보는 저장하지 않으며, 이 기록은 회원 계정과 연결하지 않습니다',
        '페이지 방문 시',
      ],
    },
    {
