...
const COLLECTION_TABLE: PolicyTable = {
  ...
  rows: [
    ...
    // 저장하는 항목은 전부 방침에 적혀 있어야 하므로 자동 생성 항목에서 따로 떼어 명시한다.
    {
      cells: [
        '자동 생성 (방문 통계)',
        '익명 방문자 ID (브라우저가 생성한 무작위 값), 방문한 페이지 경로, 브라우저·기기 종류(User-Agent), 회원/게스트 구분, 유입 출처 도메인 (예: naver.com — 전체 주소와 검색어는 저장하지 않음), 방문 일시 — 접속 IP 와 회원 개인정보는 저장하지 않음',
        '페이지 방문 시',
      ],
    },
    {
        ...
        <article className="prose prose-sm dark:prose-invert max-w-none space-y-10 leading-relaxed">
          ...
          <Section title="제2조 (개인정보의 이용 목적)">
            ...
            <Note>
              ...
              정보(User-Agent) · 회원/게스트 구분 · 유입 출처 도메인을 기록하며, IP와 회원
              개인정보는 수집하지 않음) 를 사용합니다.
            </Note>
            <Note>
          ...
          <Section title="제3조 (개인정보의 보유 및 이용 기간)">
            <Ul
              items={[
                ...
                '익명 방문 로그 (방문자 ID · 경로 · User-Agent · 회원/게스트 구분 · 유입 출처 도메인) — 90일간 보관 후 매일 자동 삭제',
