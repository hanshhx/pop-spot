 ...
 * useEffect 하나였다. 그런데 로그인 성공 경로는 <b>전부 홈</b>이었다 — 이메일·2단계 인증·소셜
 * 콜백 넷 다. 즉 <b>평범하게 로그인하면 상세 페이지를 지나지 않으므로 이전이 한 번도 돌지
 * 않았다.</b> (착지는 2026-09-06 부터 원래 있던 자리로 바뀌었다. 그래도 상세 화면에 다시 붙일
 * 이유는 없다 — 복귀 주소가 없거나 만료된 사람은 여전히 홈에 착지한다.) 담아 둔
