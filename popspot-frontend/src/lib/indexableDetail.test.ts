...
describe('서울이 아닌 것', () => {
  ...
  /*
   * 이름에 비서울 지명이 든 서울 팝업. 2026-09-05 성수동에서 여는 '제주 로컬브랜드
   * 팝업스토어' 가 실제로 이 모양이다 — 브랜드가 제주고 장소는 서울이다.
   *
   * 지금 판정은 location 만 보므로 통과한다. 여기에 검사를 박아 두는 이유는, 위의 "서울 판교"
   * 오염을 더 잡겠다고 name 까지 보도록 넓히고 싶어질 때 <b>이런 건이 통째로 검색에서
   * 사라진다</b>는 것을 그 자리에서 알려 주기 위해서다.
   */
  it('이름에 비서울 지명이 있어도 주소가 서울이면 색인한다', () => {
    expect(
      judgeIndexable(
        {
          name: '2026 제주 로컬브랜드 팝업스토어',
          location: '서울 성동구 뚝섬로13길 38',
          endDate: '2026-09-06',
        },
        TODAY,
      ),
