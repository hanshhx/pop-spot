import { SEASONS, seasonOfMonth, seasonOfNow, type Season } from './season';
 ...
 *
 * <p>이 값은 페이지 래퍼의 {@code data-season} 으로 들어간다. 그래서 globals.css 의 계절
 * 블록에 {@code :root} 없는 짝이 필요하다 — 없으면 여기서 계산한 계절이 조용히 무시된다.
 */
...
export function landingSeason(slug: string, now: Date = new Date()): Season {
  return seasonFromSlug(slug) ?? seasonOfNow(now);
}
