...
import { MessageCircle, Plus, User as UserIcon, MapPin, X, TrendingUp, Crown, Flag, Users, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-surface">
      ...
      <div className="p-4 md:p-6 border-b border-[var(--color-border)] flex justify-between items-center gap-3 bg-surface/90 backdrop-blur-md sticky top-0 z-10">
        <div>
          <h2 className="text-xl md:text-2xl font-black text-foreground tracking-tight">같이 갈 사람 구해요</h2>
          <p className="text-xs md:text-sm text-muted-foreground mt-0.5">혼자 가기 아쉬운 팝업, 동행을 찾아보세요.</p>
        </div>
        <button
          onClick={openWrite}
        ...
        {megaphonePosts.length > 0 && (
          <div className="bg-lime-300/5 dark:bg-ink-800/50 border-b border-[var(--color-border)] py-5">
            <div className="px-4 md:px-6 mb-3 flex items-center gap-2">
              ...
              <span className="font-black text-sm md:text-base text-foreground tracking-wide">상단 부스트</span>
            </div>
                ...
                >
                      ...
                      <div className="min-w-0 flex-1 pr-8">
                        ...
                        {post.targetPopup && (
                            <p className="flex items-center gap-1 text-[11px] md:text-xs font-bold text-hot-500 dark:text-hot-400 truncate">
                                <MapPin size={11} className="shrink-0"/> <span className="truncate">{post.targetPopup}</span>
                            </p>
                        )}
                      </div>
                    </div>

                    <p className="text-xs md:text-sm text-muted-foreground my-3 line-clamp-1">{post.content}</p>
                    ...
                    <div className="flex justify-between items-center mt-auto border-t border-hot-100 dark:border-hot-900/30 pt-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => handleJoinChat(post)} className="px-3 py-1.5 md:px-4 md:py-2 bg-hot-500 hover:bg-hot-400 text-white rounded-pill text-[10px] md:text-xs font-bold shadow-md shadow-hot-400/30 flex items-center gap-1 transition-all active:scale-95">
            ...
            <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
              {normalPosts.map((post) => (
                  ...
                  >
                      ...
                      <div className="flex gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1.5 mb-1">
                                  <span className={`px-1.5 py-0.5 rounded-full text-[9px] md:text-[10px] font-bold shrink-0 ${
                                  ...
                                  <span className="ml-auto text-[10px] md:text-xs text-muted-foreground shrink-0">{new Date(post.createdAt).toLocaleDateString()}</span>
                              </div>
                      ...
                      <p className="text-xs md:text-sm text-muted-foreground line-clamp-2 mt-2.5">{post.content}</p>
                          ...
                          <div className="flex items-center gap-2 md:gap-3 shrink-0">
                              ...
                              <button
                                  ...
                                  className={`px-3 py-1.5 md:px-4 md:py-2 rounded-pill text-[10px] md:text-xs font-bold flex items-center gap-1 transition-all active:scale-95 ${
                                      post.status === 'CLOSED' ? "bg-gray-500 hover:bg-gray-600 text-white" : "bg-lime-300 hover:bg-lime-400 text-ink-900"
                                  }`}
                              >
    ...
    <div className="px-1 py-8 md:py-10">
      <div className="mx-auto max-w-md text-center">
        ...
        <p className="mt-1.5 text-sm text-muted-foreground">
          혼자 가기 아쉬운 팝업이 있다면 <b className="text-lime-600 dark:text-lime-300">'동행 구하기'</b>를 눌러보세요.
      ...
      <div className="grid gap-3 sm:grid-cols-2 opacity-55 select-none pointer-events-none">
        {devMatePosts().slice(0, 2).map((post) => (
          <div key={post.id} className="p-4 rounded-2xl border border-dashed border-[var(--color-border)] bg-white dark:bg-ink-700">
            <div className="flex gap-3">
              <PopupThumb seed={post.targetPopup || post.title} size="lg" />
              <div className="min-w-0 flex-1">
                <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400">모집중</span>
                <h3 className="mt-1 text-base font-bold text-foreground truncate">{post.title}</h3>
            ...
            <div className="flex justify-between items-center border-t border-[var(--color-border)] pt-3 mt-3">
              <span className="text-[11px] font-bold text-foreground">{post.author.nickname}</span>
              <span className="text-[11px] font-bold text-muted-foreground">
                <span className="text-lime-500 mr-0.5">{post.currentPeople}</span>/ {post.maxPeople}명
              </span>
            </div>
          </div>
        ))}
