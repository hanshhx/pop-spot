  ...
  return (
    <div ref={menuRef} className={`relative inline-flex ${className}`}>
      <button
        ...
        aria-haspopup="menu"
        aria-label="Language"
        className="inline-flex min-h-11 items-center gap-2 rounded-full border border-gray-200 bg-white/90 px-3 text-xs font-black uppercase text-foreground shadow-sm backdrop-blur transition hover:border-lime-400 md:hidden dark:border-white/10 dark:bg-black/55"
      >
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 top-12 z-[70] min-w-40 overflow-hidden rounded-2xl border border-gray-200 bg-white p-1.5 text-gray-900 shadow-xl md:hidden dark:border-white/10 dark:bg-[#171717] dark:text-white"
        >
          {localeLinks(true)}
        </div>
      ) : null}
