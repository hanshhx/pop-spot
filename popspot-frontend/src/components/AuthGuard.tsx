...
const PUBLIC_PATHS = [
  ...
  '/map',
  // 계절 테마 확인용 화면. 색 견본만 있고 팝업 데이터는 없어서 가릴 것이 없는데, 막아 두면
  // 디자인 리뷰 때마다 로그인으로 튕긴다. 색인은 페이지 자체가 noindex 로 막는다.
  '/season-preview',
  // 제휴 자료 갤러리 확인용 화면. 위 계절 미리보기와 같은 처지다 — 샘플 그림뿐이라 가릴 것이
  // 없는데, 막아 두면 자료를 올릴 때마다 로그인으로 튕긴다. 색인은 페이지가 noindex 로 막는다.
  '/gallery-preview',
];
