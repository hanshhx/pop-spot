 ...
 */
function fitToPoints(map: MapLibreMap, pts: [number, number][]) {
...
export default function InteractiveMap({
  ...
  useEffect(() => {
    if (places && places.length > 0) {
      ...
      const convertedMarkers: MapMarkerData[] = places.map((p) => {
        ...
        const safeLng = p.lng !== undefined && p.lng !== null ? p.lng : 127.056;
  ...
  const markers = useMemo(() => {
    ...
    if (activeCategory !== 'ALL') {
      filtered = filtered.filter((m) => (m.category ?? '').toUpperCase() === activeCategory);
    ...
    if (activePeriod) {
      filtered = filtered.filter((m) => matchesPeriod(m.startDate, m.endDate, activePeriod));
    }
  ...
  const getCategoryStyle = (category?: string) => {
    ...
    return {
      color: meta.text,
      border: meta.border,
      icon: <Icon className="w-2.5 h-2.5 md:w-3 md:h-3" />,
    };
  };
                ...
                >
                  {showPath || mode === 'PLAN' ? (
                    <div className="relative flex flex-col items-center hover:z-50">
                      ...
                      <div
                        className={`flex items-center gap-1.5 md:gap-2 px-2 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl shadow-lg border-2 bg-white ${style.border} mb-1 transform transition-all group-hover/marker:scale-110`}
                      >
                        <span className={`font-black text-[10px] md:text-xs ${style.color}`}>
                          {index + 1}
                        </span>
                      ...
                      <div className="relative">
                        <div
                          className={`w-3 h-3 md:w-4 md:h-4 transform rotate-45 border-r border-b bg-white ${style.border} -mt-2 md:-mt-3`}
                        ></div>
                    ...
                    <div className="relative flex flex-col items-center hover:z-50">
                      ...
                      >
                        <span
                          className={
                            selectedMarker?.popupId === marker.popupId ? 'text-black' : style.color
                          }
                        >
                          {style.icon}
                        </span>
                      ...
                      {/* 카드 아래 작은 점 (위치 표시) */}
                      <div
                        className={`w-2 h-2 mt-0.5 rounded-full border-2 border-gray-400 dark:border-black ${
