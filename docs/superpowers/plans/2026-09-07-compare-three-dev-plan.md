...
- [x] 8. 배포 후 운영에서 **비회원** 확인 완료(§10). 회원 화면과 실기기 터치가 남았다

## 9. 운영 배포 확인 (2026-09-07)

Vercel 자동 배포. **바이트가 갔다는 것과 실제로 돈다는 것을 나눠서 확인했다.**

##### 1) 번들에 들어갔는가

홈 페이지가 부르는 JS 청크 20개를 훑어 두 문자열을 찾았다.

| 문자열 | 청크 |
|---|---|
| `popspot:compare:selected` | `25e010u28ydbu.js` |
| `나란히 보기` | `1zrdxxvivzi7l.js` |

크롤러 jar 를 올릴 때 `unzip -p … \| grep` 으로 프롬프트를 대조한 것과 같은 방법이다 —
CI 초록과 배포 성공은 "내가 의도한 문자열이 그 바이트에 있다" 를 증명하지 않는다.

##### 2) 실제로 도는가 (popspot.co.kr, 비회원)

찜 3건을 심고 걸었다.

| 확인 | 결과 |
|---|---|
| 저장소 | `[957,1043]` → `[957,1043,1350]` |
| 진입 버튼 | "2곳 나란히 보기" (개수 연동) |
| 모달 | 열림 · 제목 "나란히 보기" |
| 순서 | `D-19` → `D-22` → `마감일 미정` |
| 기간 칸 | `07-09 ~ 09-27` · `07-18 ~ 09-30` · **`07-15 ~`** |
| 한 줄 소개 | 셋이 서로 다름 |

세 번째 기간 칸 `07-15 ~` 가 이 기능의 핵심 판단이 운영에서 작동한 증거다.
종료일 없는 64.5% 를 `-` 로 깎았으면 저 칸이 비었을 것이다.

##### ⚠️ 운영에서 못 잰 것 — z-index

`document.elementFromPoint` 검사를 운영에서 돌렸더니 `0/9` 가 나왔다. **그런데 이것은
버튼이 안 눌린다는 뜻이 아니다.** 반환값이 `null` 이었고 `innerWidth × innerHeight` 가
`0 × 0` 이었다 — 브라우저 창이 가려져 페이지가 렌더링되지 않는 상태였다. 레이아웃이
없으면 좌표에 아무것도 없다. 가려진 것이라면 덮고 있는 `A` 태그가 나왔을 것이다
(2026-09-06 의 실제 버그가 그랬다).

**같은 함정을 이 계획서가 이미 기록해 뒀다** — §7.1 의 "숨겨진 창에서 잰 애니메이션
의존 동작은 근거가 되지 않는다". `elementFromPoint` 도 같은 부류다.

그래서 운영 z-index 근거는 **개발 서버에서 렌더된 채로 잰 9/9** 다. 운영 번들에 같은
코드가 들어 있는 것은 위에서 확인했다. 사람이 실제 기기에서 눌러 보는 것이 남았다.

##### 남은 것

- [ ] 회원 계정으로 같은 걸음(비회원만 확인했다)
- [ ] 실제 기기에서 카드 우하단 버튼을 손가락으로 눌러 보기
