 ...
 * 관리자가 고른 값을 담는 키.
 *
 * <p>웹은 이 값을 쿠키에 두고 서버 컴포넌트가 읽어 첫 HTML 부터 계절을 입힌다. 앱에는 서버 렌더가
 * 없으므로 로컬 저장소 키가 된다. <b>이름과 값의 뜻은 웹과 같게 둔다</b> — 나중에 계절 설정을
 * 계정에 붙일 때 두 곳이 같은 문자열을 주고받아야 하고, 이름이 갈리면 그때 한쪽만 고쳐진다.
 */
export const SEASON_STORAGE_KEY = 'popspot-season';
