...
## 웹에서 수집하면 앱에 반영되나 — 된다, 다만 조건이 있다

같은 DB, 같은 `/api/popups` 다. 관리자에서 「지금 수집하기」(`POST /api/admin/popups/crawl/run`)를
누르면 앱도 그것을 본다. **앱에서 따로 할 일은 없다.** 다만 즉시는 아니고, 네 관문을 통과해야 한다.

| 관문 | 걸리면 |
|---|---|
| `reviewStatus` 가 `AUTO_PUBLISHED`/`APPROVED` | 검수 대기(`PENDING_REVIEW`)는 목록 API 에 아예 안 나온다 |
| 서버 캐시 `max-age=300` | 최대 5분 |
| 앱 스토어 캐시 `STALE_MS` (5분) | 화면을 옮겨도 5분 안에는 안 다시 받는다 |
| `derive()` 4단계 | 날짜 없음·종료·서울 밖·좌표 없음이면 안 보인다 |

**바로 보려면** 홈의 「목록 새로고침」을 누른다 — `reload()` 는 신선도를 보지 않고 무조건 다시 받는다.

## 목록은 한 번만 받는다

`usePopups` 는 원래 평범한 훅이라 **그것을 부르는 화면 열 곳이 각자** `/api/popups` 를 받았다.
응답이 **1,347,991바이트**다(실측 2026-08-30). 홈 → 상세 → 전체보기 → 일정만 눌러도 5.4MB 를
이동통신망으로 내려받고, 화면을 열 때마다 목록이 비었다가 채워져 카드가 깜빡였다.

지금은 `store/usePopupStore.ts` 가 한 번 받고 파생 계산(`derive`)도 목록이 바뀔 때 한 번만 돈다.
훅에 두면 화면 열 곳이 1,455건을 각자 훑는다. 동시에 마운트돼도 `inFlight` 로 요청은 하나다.
...
### App Links — 다음 네이티브 빌드에서 켜진다

커스텀 스킴(`popspot://`)은 독점이 아니라 다른 앱도 등록할 수 있다. 그래서 콜백은 이제
**`https://popspot.co.kr/app/auth`** 로 넘긴다. 이 주소는 두 가지로 열린다:

- **App Links 가 검증된 기기** — 안드로이드가 그 페이지를 열지 않고 앱에 바로 넘긴다. 가로챌 수 없다.
- **그 밖(지금 깔린 빌드 전부)** — 브라우저가 1KB 짜리 페이지를 열고 거기서 `popspot://` 로 넘긴다.

즉 **지금도 되고, 다음 빌드에서 저절로 더 안전해진다.** 앱 JS 도 웹도 그때 고칠 것이 없다
(`parseAuthDeepLink` 가 두 접두사를 이미 받는다).

남은 것은 **지문 하나**뿐이다. `/.well-known/assetlinks.json` 에 지금 EAS 업로드 키
(`38:28:CA:98:…`)가 올라가 있는데, 스토어에서 받은 앱은 **구글이 다시 서명한 키**로 서명돼 있다.
Play Console → 앱 무결성 → 앱 서명 키 인증서의 SHA-256 을 Vercel 환경변수
`ANDROID_CERT_FINGERPRINTS` 에 넣으면 된다(쉼표로 여러 개). **코드 배포 없이 값만 고치면 된다** —
그러라고 정적 파일이 아니라 라우트로 만들었다.

지문은 폰에 깔린 APK 서명 블록에서 직접 뽑을 수도 있다(v2 전용 서명이라 `keytool -printcert
-jarfile` 로는 안 읽힌다 — "Not a signed jar file" 이 나온다).
...
- **2단계 인증은 이제 된다.** `features/auth/TotpScreen.tsx` — 이메일·소셜 두 경로가 **같은 화면**을
  쓴다(웹이 `TotpChallenge` 를 두 경로에 공유하는 것과 같은 이유: 따로 만들면 한쪽만 고치는 사고가
  난다). 표는 한 번 쓰면 서버에서 사라지므로, 코드를 틀리면 재입력이 무조건 실패한다 — 입력칸을
  잠그고 "다시 로그인" 으로 보낸다. 복구 코드 입력도 함께 받는다.
- **아직 남은 것은 App Links 지문 하나뿐이다**(위 절).
...
- **App Links 검증** — 설정과 호스팅은 끝났고 **Play 앱 서명 지문**만 넣으면 된다(위 절). 그전까지는
  커스텀 스킴으로 돌아오고, 그 경로는 난수로 막아 두었다.
- **소셜 로그인 자체는 된다.** 다만 `.well-known` 검증이 붙기 전에는 스킴 가로채기의 이론적 여지가
  남는다.
