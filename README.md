...
### v2.28 — UI 정리 (검색 일원화 · 네비 축소 · 색/여백 다듬기)

> 사용성 피드백 반영: 중복 검색·네비 정리 + 가독성/여백 손질.

| 항목 | 변경 |
|---|---|
| 검색 | 헤더 "통합검색" 제거 → **SearchZone 하나로 일원화** |
| 네비 | BottomDock "의견" 탭 제거 → **MY 탭 안 "의견·문의 보내기"로 통합** |
| 캘린더 카드 | 상단 중복 캘린더 아이콘 제거 |
| 색 | 웰컴배너 "N개" 톤다운(lime-300→400/700) · LIVE NOW 팝업명 흰색→잉크(가독성) |
| 프로필 | 업로드 안내 "jpg/png/webp" 제거, "최대 5MB"만 |
| 여백 | 헤더·대시보드·LIVE NOW 하단 마진 축소 |

<sub>프론트 빌드 41/41 통과. 데스크톱 네비 개편·모바일 짤림·정밀 여백·팝업 상세 SEO 는 다음 버전에서 이어서. 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.31 참고.</sub>

### v2.29 — SEO: 지역×카테고리 조합 랜딩 대량 생성 (약관 §10-2 준수)

> GSC 노출 저조 진단 = 색인 표면이 슬라이스 23개뿐. 개별 팝업은 약관 §10-2(자동수집 재배포 금지)로 **의도적 비색인 유지**. 대신 큐레이션 **조합 랜딩**을 늘려 롱테일 표면 확장.

| 항목 | 내용 |
|---|---|
| 조합 랜딩 | `/popups/[지역]-[카테고리]` (예: `/popups/seongsu-fashion` = "성수 패션 팝업") 지역×카테고리 자동 생성. 슬라이스 **23 → ~100개** |
| 필터/메타 | 지역+카테고리 동시 매칭. title·H1·intro·FAQ·JSON-LD 모두 조합 키워드 |
| 품질 안전 | 결과 0곳 조합은 **noindex**(thin content 방지, 접근·내부링크는 유지) |
| sitemap | 조합 랜딩 등록. 자동수집 개별 팝업은 §10-2 준수로 계속 제외 |

<sub>프론트 빌드 통과(~100 슬라이스 SSG). 배포 후 GSC 에서 sitemap 재제출 + 조합 페이지 색인 요청 권장. 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.32 참고.</sub>

### v2.30 — 데스크톱 네비 분리 (웹=상단 네비 / 모바일=하단 독)

> "웹에서 바텀독이 모바일틱하다" 피드백 반영. 데스크톱(lg+)은 헤더 가로 네비, 모바일/태블릿은 기존 BottomDock.

| 항목 | 변경 |
|---|---|
| 데스크톱(lg+) | 헤더에 6탭 가로 네비(지도·코스·음악·여권·MY·동행) + 현재 탭 하이라이트 |
| 모바일/태블릿 | BottomDock 유지, 데스크톱에선 `lg:hidden` 으로 숨김 |
| 여백 | 데스크톱 하단 패딩 `pb-32 → lg:pb-16` (독 없는 데스크톱 여백 축소) |

<sub>미리보기 실측 검증: 데스크톱=헤더네비 표시·독 숨김 / 모바일=독 표시(6탭)·헤더네비 숨김 · 가로 오버플로우 0. 빌드 118/118 통과. 모바일 "짤림"은 뷰포트 오버플로우 없음으로 측정됨(백엔드 미가동이라 데이터 의존 클리핑은 미확인). 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.33.</sub>

### v2.30.1 — 네비게이션 UI 개선 (크기 ↑ · 라임 액티브 · 캡슐 바)

> "네비가 대충한 것 같다" 피드백. 모바일 독 + 데스크톱 네비 둘 다 다듬고 키움.

- **모바일 독**: 버튼 높이 48→**56px** · 6탭 균등 배치(가로 스크롤 제거) · 액티브 = **라임 캡슐(lime-300)** · 아이콘 size-5→6, 라벨 확대 · 컨테이너 라운드↑ + `backdrop-blur-xl` + subtle ring
- **데스크톱 네비**: **캡슐 바**로 감싸고(pill 컨테이너) 패딩·아이콘 확대 · 액티브도 라임으로 통일

<sub>미리보기 실측: 독 버튼 56px·액티브 lime-300·가로 오버플로우 0 / 데스크톱 캡슐 네비 lime 액티브. 빌드 통과. 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.34.</sub>

### v2.30.2 — 데스크톱 네비 텍스트형 재설계 (레퍼런스 반영)

> "데스크톱 네비가 하단 독을 그대로 위로 올린 느낌"이라는 피드백. 일반 웹 상단 메뉴처럼 **텍스트 메뉴**로 재설계.

- 데스크톱 네비: 아이콘·캡슐 제거 → **텍스트 메뉴**(지도·코스·음악·여권·MY·동행), 액티브 = **라임 언더라인 + 볼드**, 헤더에 자연스럽게 통합
- 모바일 하단 독은 유지

<sub>미리보기 실측: 데스크톱 네비 아이콘 0개·텍스트 라벨·액티브 lime-400 언더라인(font-weight 700). 빌드 통과. 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.35.</sub>

### v2.30.3 — 네비게이션 크기 확대

> "네비가 너무 작다" 피드백. 모바일 독 + 데스크톱 네비 둘 다 확대.

- **모바일 독**: 버튼 56→**64px**(md 72px), 아이콘 size-6→7, 라벨 11→12px, 바 폭 max 520→560
- **데스크톱 네비**: 텍스트 15→**17px**, 간격 gap-7→8, py-1→1.5
...
### v2.33 — 자동수집 다건 추출 (하루 8건 → 수십 건, 스크래핑·쿼터 추가 0)

> 진단: 크롤러가 키워드당 API 로 최대 120개 snippet 을 가져오면서 **앞 8개만 LLM 에 투입**(`MAX_SNIPPETS=8`)하고, 프롬프트도 **"팝업 1개만 출력"** 이라 키워드당 팝업 1개가 상한이었다. 85키워드 × 1 − 중복/저신뢰 = **하루 ≈8건**. → 데이터 90~120개에서 정체.

| 항목 | 변경 |
|---|---|
| 추출 | `normalize()`(1개) → **`normalizeAll()`(배열 — 서로 다른 팝업 전부)** |
| 프롬프트 | "동일 팝업 1개" → "서로 다른 팝업 **모두** JSON 배열 + `sourceIndex`" |
| snippet 상한 | 8 → **40** (이미 가져온 데이터 활용, API 호출·쿼터는 그대로) |
| 소스 편향 | 4채널 순차 concat → **라운드로빈 교차**(앞 40개가 네이버·카카오 골고루) |
| 출처 매핑 | `sourceIndex` 로 팝업별 근거 snippet 을 sourceUrl 에 연결 |
| 통계 | `llmCalls`(호출 수) / `normalized`(후보 수) 분리 로깅 |

<sub>LLM 호출 수는 **키워드당 1회 그대로** → Groq RPM·API 쿼터 추가 소모 0. 신뢰도 0.8 게이트·중복제거(externalId)·PII 규칙 유지. 공식 네이버·카카오 검색 API 내(본문 스크래핑 X) → §10-2·저작권·쿼터 정책 무위반. ⚠️ **백엔드 변경** — 로컬 Gradle loopback 이슈로 미컴파일, 배포 전 `./gradlew spotlessApply build -x test` 필수. 자세한 변경은 `PROJECT_CHANGELOG.md` ch.29.39.</sub>

### v2.33.1 — 다건 추출 정확도 가드

> v2.33 다건 추출의 부작용(교차오염·유사중복) 방어. 프롬프트에 규칙 2개 추가 — **"확실하지 않으면 제외(물량보다 정확도 우선)"** + **"한 팝업 정보를 다른 팝업에 섞지 마(교차오염 금지)·유사중복은 병합"**. 신뢰도 0.8 게이트는 그대로. ⚠️ 백엔드 — v2.33 과 함께 배포. `PROJECT_CHANGELOG.md` ch.29.40.
