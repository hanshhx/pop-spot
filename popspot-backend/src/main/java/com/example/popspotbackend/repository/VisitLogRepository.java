...
public interface VisitLogRepository extends JpaRepository<VisitLog, Long> {
    ...
    @Query(
            value =
                    "SELECT visitor_id, COUNT(*) AS visits, STRING_AGG(DISTINCT path, ', ') AS"
                        + " paths, MAX(created_at) AS last_seen, BOOL_AND(guest) AS all_guest,"
                        + " MAX(user_agent) AS ua FROM visit_log WHERE created_at >= :since GROUP"
                        + " BY visitor_id ORDER BY MAX(created_at) DESC LIMIT 100",
            nativeQuery = true)
