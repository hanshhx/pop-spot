...
public interface VisitEventRepository extends JpaRepository<VisitEvent, Long> {
     ...
     *
     * <p><b>사람 수를 센다. 횟수가 아니다.</b> 퍼널은 "100명 중 몇 명이 다음으로 갔나" 를 묻는
     * 것이라, 한 사람이 팝업 스무 개를 연 것과 스무 명이 하나씩 연 것이 같아 보이면 안 된다.
     *
     * <p>단계 순서·이름은 여기서 정하지 않는다. 종류별 사람 수만 돌려주고, 어떤 순서로 세울지는
     * {@code VisitService} 가 정한다 — 순서가 SQL 에 박히면 단계를 바꿀 때마다 쿼리를 고쳐야 한다.
     *
     * <p><b>가장 이른 기록 시각을 함께 준다.</b> 행동 종류는 나중에 늘어날 수 있고(실제로
     * {@code wishlist_add} · {@code outbound_click} 이 C-4 에서 추가됐다), 그 종류는 배포 전 기간에
     * 대해 0 이다. 그 사실을 모르면 <b>퍼널이 그 칸에서 뚝 끊긴 것처럼</b> 보인다. 설정으로 날짜를
     * 적어 두는 방법도 있지만 그러면 배포 때마다 사람이 손대야 하고 잊으면 또 틀린다 — 데이터에
     * 물어보면 저절로 맞는다.
     *
     ...
     *
     * <p>위 {@code visitorsByType} 의 최소값은 조회 구간 안에서만 본 것이라, 구간보다 앞서 수집이
     * 시작됐으면 "구간 시작일" 이 나온다. 그러면 처음부터 모아 온 단계까지 "최근에 시작됨" 으로
     * 보인다. 수집 시작일은 구간과 상관없는 사실이므로 따로 묻는다.
     */
    @Query(
            value =
                    "SELECT MIN(e.created_at) FROM visit_event e WHERE e.event_type = :eventType",
            nativeQuery = true)
     ...
     *
     * <p>퍼널의 마지막 칸(재방문)이다. 같은 세션 안의 여러 행동은 재방문이 아니므로 세션이 달라야
     * 한다 — 이 조건을 빼면 한 번 왔다 간 사람이 전부 재방문으로 잡혀 퍼널 끝이 부풀려진다.
     *
     * <p>{@code session_id} 가 없는 옛 기록은 세션을 비교할 수 없어 빠진다. 그만큼 <b>낮게</b>
     * 나오지 높게 나오지는 않는다.
     */
    @Query(
            value =
