...
public interface PopupStoreRepository extends JpaRepository<PopupStore, Long> {
     ...
     *
     * <ul>
     *   <li><b>같은 대상을 매번 다시 훑는다.</b> {@code lastSeenAt} 오름차순이라 기간을 못 찾은 팝업이
     *       다음 회차에도 맨 앞에 그대로 남는다. 실측에서 80건 중 65건이 매번 실패했고, 그 65건이 입구를
     *       막아 나머지 800여 건에는 <b>영영 도달하지 못한다.</b>
     *   <li><b>가장 값어치 낮은 순서다.</b> v2.45 부터 새 팝업은 날짜가 있어야 저장되므로, 결손 목록에
     *       남은 것은 대부분 옛날에 끝난 팝업이다. 채워도 "이미 종료" 로 사라진다.
     * </ul>
     *
     * <p>그래서 <b>가치로 나눈 뒤 그 안에서 무작위</b>로 뽑는다.
     *
     * <ul>
     *   <li>0순위 — 시작일은 있고 <b>종료일만 빈</b> 팝업. 지금 화면에 보이고 있어(v2.44 isOpenNow 통과)
     *       종료일이 채워지면 "언제 끝나는지" 를 바로 답할 수 있다. 실측 883건 중 323건.
     *   <li>1순위 — 나머지(둘 다 없음 등).
     * </ul>
     *
     * <p>무작위가 필요한 이유는 순환이다. 우선순위만 두면 그 안에서 또 같은 것만 보고, 무작위만 쓰면
     * 값어치 낮은 옛 팝업에 예산을 쓴다. 둘을 겹쳐야 매 회차 다른 대상을 보면서도 값어치 순서를 지킨다.
     *
     * <p>기존 메서드를 고치지 않고 새로 만든 것은 그쪽을 LLM 재처리 큐({@code queueMissingDateSources})가
     * 함께 쓰기 때문이다 — 정렬을 바꾸면 그 동작까지 조용히 달라진다.
     */
    @Query(
            value =
                    """
