 ...
 * userAgent} 는 봇 식별/제외 목적으로만 저장하는 브라우저·기기 종류 문자열(개인 식별 불가)이다. {@code referrerHost} 는 유입 출처의 도메인 또는
 * 분류값만 남긴다(전체 URL 미저장 — 개인 식별 불가).
 */
@Entity
@Getter
@Builder
...
public class VisitLog {
    ...
    /**
     * 유입 출처 — 도메인(예: {@code search.naver.com}) 또는 분류값(direct=주소 직접 입력·북마크, internal=사이트 내 이동). 전체
     * URL 은 쿼리스트링에 검색어·개인정보가 실릴 수 있어 저장하지 않는다. 이 기능 배포 이전 방문은 출처를 알 수 없어 NULL 이다.
     */
