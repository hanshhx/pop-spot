 ...
 *
 * <p>실시간 채팅은 동시 접속자가 있어야 돌지만, 이 제보는 혼자 눌러도 다음 사람에게 가치가 남는 <b>비동기 신호</b>다.
 * 글쓰기 없이 버튼 한 번이라 참여 문턱이 매우 낮다.
 *
 * <p>{@code waitLevel} 0 = 바로 입장, 1 = 조금 대기, 2 = 많이 대기.
 *
 * <p>{@code reporterKey} 는 로그인 유저면 {@code u:userId}, 게스트면 {@code g:익명visitorId}(개인 식별 불가).
 * 중복 제보 제한(쿨다운)에만 쓴다.
 */
@Entity
@Getter
