 ...
 *
 * <p><b>각 칸은 사람 수다. 횟수가 아니다.</b> 퍼널이 묻는 것은 "100명 중 몇 명이 다음으로 갔나" 라, 한 사람이 팝업 스무 개를 연 것과 스무 명이 하나씩 연
 * 것이 같아 보이면 안 된다.
 *
...
public record FunnelDto(List<Step> steps, String since, String note) {
     ...
     * @param collectedSince 이 단계의 수집이 시작된 날. 그전 기간은 0 으로 보인다. 처음부터 모은 단계는 빈 문자열
     */
    public record Step(
            String key,
            String label,
     ...
     *
     * <p><b>비율을 화면에 미루지 않는다.</b> 분모가 0 인 자리가 칸마다 있어서, 화면에서 나누면 그 자리를 화면 수만큼 다시 만들게 된다. 방문이 없는 날 관리자
     * 표에 {@code NaN} 이 뜨면 "집계가 고장났다" 로 읽힌다.
     *
     * <p><b>퍼널이 아래로 갈수록 줄어든다고 가정하지 않는다.</b> 상세를 연 사람보다 찜한 사람이 많을 수는 없어야 정상이지만, 수집 시작일이 단계마다 다르거나
     * 링크를 직접 받아 들어온 경우 뒤 칸이 앞 칸보다 클 수 있다. 그때 100% 를 넘겨 그대로 보여 준다 — 억지로 자르면 <b>이상하다는 신호</b> 까지 지워진다.
     */
