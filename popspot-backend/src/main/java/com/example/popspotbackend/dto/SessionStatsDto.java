 ...
 *
 * <p><b>왜 필요했나.</b> 수집 쪽은 이미 {@code visit_event.session_id} 를 채우고 있었는데, 그것을
 * 읽는 관리자 API 가 하나도 없었다. 원재료만 쌓이고 조리를 안 한 상태였다. 그래서 방문자 CSV 로는
 * 재방문율을 계산할 수 없었고, 유입만 늘리고 <b>남는지는 모르는</b> 상태가 이어졌다.
 *
...
public record SessionStatsDto(
     ...
     *
     * <p><b>나눗셈을 화면에 미루지 않는다.</b> 프론트에서 나누면 0 으로 나누는 자리가 화면 수만큼
     * 생기고, 반올림 자리도 화면마다 달라진다. 여기서 한 번만 정한다.
     *
     * <p>재방문자가 방문자보다 많을 수는 없지만, 두 쿼리가 서로 다른 순간을 보면(집계 중 새 방문이
     * 들어오면) 그런 값이 나올 수 있다. 그때 신규가 음수가 되면 표가 이상해지므로 0 에서 자른다.
     */
    public static SessionStatsDto of(
            long sessions, long visitors, long events, long returning, int retentionDays,
            boolean truncated) {
