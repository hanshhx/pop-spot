 ...
 *
 * <p>v2.46 — 1차 수집은 검색 API 의 짧은 요약문만 본다. 실측(호출 30회·팝업 76개)에서 날짜를 못 뽑은
 * 34건 중 <b>24건(71%)이 "요약문에 기간이 아예 없어서"</b> 였다. 프롬프트로는 없는 정보를 만들 수 없어,
 * 결손분만 골라 원본 글을 한 번 더 읽는다.
 *
 * <p>기대치는 부풀리지 않는다 — 원본 19건을 직접 열어 보니 기간 표현이 있는 것은 <b>5건(26%)</b>이었다.
 * 나머지는 포스터 <b>이미지</b>에 박혀 있어 글자로는 못 읽는다. 즉 26% 가 이 방식의 천장이다.
 *
 * <p>비용은 거의 없다. 본문을 LLM 에 넘기지 않고 정규식으로 기간만 뽑는다
 * ({@link PopupBodyDateExtractor}) — 실측에서 정규식만으로 충분했다.
 *
 * <p><b>틀린 날짜를 넣느니 비워 둔다.</b> v2.45 이후 날짜 없는 팝업은 화면에 안 나오므로, 잘못 채우면
 * 닫힌 팝업이 열린 것처럼 보인다. 추출기가 애매한 후보를 전부 버리도록 만든 이유다.
 */
@Slf4j
@Service
...
public class PopupDateBackfillService {
    ...
    public Map<String, Integer> runOnce() {
        List<PopupStore> targets =
                popupStoreRepository.findCrawledMissingDates().stream().limit(Math.max(1, limit))
                        .toList();
        ...
        log.info("[BodyDateBackfill] 시작 — 대상 {}건", targets.size());
        ...
        for (PopupStore popup : targets) {
            ...
            Optional<PopupBodyDateExtractor.Period> period = extractor.extract(body.get(), reference);
            if (period.isEmpty()) {
                noPeriod++;
                continue;
            }
        ...
        log.info(
                "[BodyDateBackfill] 완료 — 대상 {} / 본문조회 {} / 채움 {} / 기간없음 {}",
                targets.size(),
                fetched,
                filled,
                noPeriod);
     ...
     *
     * <p>1차 수집이 뽑은 값이 본문에서 읽은 것보다 정확할 수 있고(그 팝업을 직접 가리킨 문장에서 뽑았다),
     * 덮어쓰면 되돌릴 방법이 없다.
     */
