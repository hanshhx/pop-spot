 ...
 *
 * <p>기존에는 클라이언트가 payload 로 보낸 sender 를 그대로 저장해, 로그인 없이 아무 이름이나(심지어 타 회원·운영자) 사칭할 수 있었다.
 * 핸드셰이크({@code WebSocketConfig.JwtHandshakeInterceptor})가 JWT 를 검증해 세션에 넣어 둔 {@code userId} 로 실제
 * 닉네임을 강제하고, 인증되지 않은 연결은 "익명" 으로 고정한다. 클라이언트가 보낸 sender 는 신뢰하지 않는다. 세션당 최초 1회만 조회하고 캐시한다.
 */
@Component
