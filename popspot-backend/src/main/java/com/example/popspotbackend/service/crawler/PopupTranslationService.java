 ...
 *
 * <p><b>번역이 아니라 되돌리기다.</b> 한국 팝업 이름의 상당수는 외국 브랜드·IP를 한글로 적어 둔 것이라
 * (진격의 거인 = Attack on Titan, 에르메스 = Hermès, 니텐도 = Nintendo), 외국인이 읽어도 자기가 아는
 * 이름인 줄 모른다. 음역하면 아무 소용이 없고 원래 이름을 찾아 줘야 한다.
 *
 * <p>실제 이름 60건으로 미리 재 본 결과 87%가 그대로 쓸 만했다. 남은 실패는 대부분 <b>확신 없이
 * 지어낸 이름</b>이었고, 그중에는 관광객을 다른 장소로 보내는 것도 있었다(현대백화점 → The Hyundai 는
 * 여의도의 별개 매장이다). 그래서 확신이 낮으면 <b>비워 두고 한국어 원문을 그대로 보여준다.</b>
 *
 * <p>같은 측정에서 드러난 더 중요한 사실 — 번역명은 무엇인지 알려주는 데는 좋지만 <b>찾는 데는 못
 * 쓴다.</b> 지도 앱에 "Knotted World" 를 넣으면 안 나오고 직원에게 말해도 통하지 않는다. 그래서 화면은
 * 번역명과 원문을 <b>함께</b> 보여준다. 이 서비스가 원문을 절대 덮어쓰지 않는 이유이기도 하다.
 */
@Slf4j
@Service
...
public class PopupTranslationService {
     ...
     * 한 번에 넘길 팝업 수.
     *
     * <p>이름은 스니펫보다 훨씬 짧아 크롤 정규화(5건)보다 크게 잡을 수 있다. 다만 너무 키우면 응답이
     * 길어져 뒤쪽 항목의 품질이 떨어지고, 한 건이 깨지면 배치 전체를 잃는다.
     */
    ...
    public record Translated(
            String nameEn, String nameJa, String locationEn, String locationJa) {
     ...
     * 팝업 묶음을 번역한다.
     *
     * <p>실패해도 예외를 밖으로 내보내지 않는다 — 번역은 <b>있으면 좋은 것</b>이라, 못 했다고 수집이나
     * 백필 전체가 멈추면 안 된다. 못 한 건은 결과에서 빠지고 호출자가 원문을 그대로 쓴다.
     *
    ...
    public Map<Long, Translated> translate(List<PopupStore> popups) {
        ...
        for (int from = 0; from < popups.size(); from += BATCH_SIZE) {
            List<PopupStore> batch = popups.subList(from, Math.min(from + BATCH_SIZE, popups.size()));
            try {
                result.putAll(translateBatch(batch));
            } catch (Exception e) {
                // 배치 하나가 깨져도 나머지는 계속한다.
                log.warn(
                        "[PopupTranslation] 배치 실패({}건) — 건너뜀: {}",
                        batch.size(),
                        e.getMessage());
            }
        }
    ...
    private Map<Long, Translated> translateBatch(List<PopupStore> batch) {
        ...
        if (!selection.local() && usageTracker.isDailyQuotaExhausted(LlmUsageTracker.Role.CRAWLER)) {
     ...
     * 프롬프트.
     *
     * <p>지시가 긴 이유는 <b>측정에서 드러난 실패를 하나씩 막기 위해서</b>다. 음역으로 흐르는 것,
     * 공식 행사명을 무시하고 조합하는 것, 고유명을 업종명으로 풀어 버리는 것("한복상점" → "Hanbok
     * Shop" 은 서울에 수백 개 있는 업태명이 된다) 이 실제로 나왔다.
     */
     ...
     * 한 칸을 꺼내 쓸 만한지 본다.
     *
     * <p>원문과 같은 값은 버린다 — 번역하지 못했다는 뜻이고, 저장해 두면 화면이 "번역이 있다"고 판단해
     * 원문 병기를 건너뛴다. 한글이 남아 있는 것도 버린다(영어 칸에 한글이 섞이면 옮기다 만 것이다).
     */
    ...
    private static final java.util.regex.Pattern HANGUL =
     ...
     * 번역 결과를 엔티티에 반영한다.
     *
     * <p>{@code translatedAt} 은 <b>결과와 무관하게</b> 찍는다. 확신이 없어 비워 둔 행과 아직 안 해 본
     * 행을 구분하지 않으면, 백필이 같은 행을 영원히 다시 시도한다.
     */
