 ...
 *
 * <p>재고 키 {@code ticket:stock:{itemId}} 는 {@value #STOCK_TTL_HOURS}시간 뒤 만료된다. itemId 마다 키가 새로 생기는데
 * 만료가 없으면 이메일 인증코드·수집 예산과 공유하는 Redis 에 쓰레기 키가 무한히 쌓이기 때문. 게임 중에는 리셋/예약 때마다 TTL 을 갱신하므로 플레이 도중 사라지지
 * 않는다.
 */
@Slf4j
@Service
