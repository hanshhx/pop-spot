...
/**
 * 동행 게시판 상단 부스트 정책. 사용자 스탬프 누적량으로 등급을 결정하고, 등급별 월 한도를 돌려준다.
 *
 * <p>프론트엔드의 {@code src/lib/rank.ts} 와 같은 임계값을 쓴다. 한쪽만 바꾸면 사용자가 보는 등급과 서버가 차감하는 한도가 어긋날 수 있으므로, 임계값
 * 변경 시 양쪽 모두 동시에 갱신할 것.
 */
