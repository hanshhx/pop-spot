 ...
 *
 * <p>로그는 파일·수집기·백업으로 퍼지고 보존 기간도 길다. 화면에 안 내보내는 값이라도 로그에 남으면 결국 같은 것이 유출된다.
 *
 * <p>마스킹 규칙이 {@code AuthController} 와 {@code EmailService} 에 각각 복사돼 있었다. 한곳에 모아 규칙이 갈리지 않게 한다.
 */
public final class LogSafe {
     ...
     * </pre>
     *
     * <p>{@code Detail} 줄에 <b>위반한 값이 통째로</b> 들어간다. 그대로 로그에 남기면 가입한 이메일· 전화번호가 로그 파일에 쌓이고, 로그를 볼 수
     * 있는 사람에게는 "이 번호가 가입돼 있는가" 를 확인하는 조회 도구가 된다.
     *
