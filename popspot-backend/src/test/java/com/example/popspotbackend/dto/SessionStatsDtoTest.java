 ...
 *
 * <p>이 계산을 화면이 아니라 여기서 하는 이유가 그대로 시험 항목이다 — 분모가 0 인 자리가 셋이고,
 * 화면마다 나누면 그 셋을 화면 수만큼 다시 만들게 된다. 방문이 없는 날 관리자 표에 {@code NaN} 이나
 * {@code Infinity} 가 뜨면 "집계가 고장났다" 로 읽힌다.
 */
class SessionStatsDtoTest {
    ...
    /**
     * 집계 중에 새 방문이 들어오면 두 쿼리가 서로 다른 순간을 볼 수 있다. 그때 재방문자가 방문자보다
     * 많게 나오면 신규가 음수가 되어 표가 이상해진다 — 화면이 아니라 여기서 막는다.
     */
    @Test
    ...
    /**
     * 재방문 판정은 보관기간 안에서만 가능하다. 90일 보관인데 90일 구간을 물으면 앞쪽에는 비교할
     * 과거가 없어 그 사람들이 전부 신규로 잡힌다. 그 사실이 값에 담기지 않으면 <b>화면은 멀쩡한데
     * 숫자만 틀린다</b> — 보는 사람이 알 방법이 없다.
     */
    @Test
