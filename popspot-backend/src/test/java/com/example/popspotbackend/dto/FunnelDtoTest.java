 ...
 *
 * <p>이 계산을 화면이 아니라 여기서 하는 이유가 그대로 시험 항목이다 — 분모가 0 인 자리가 칸마다 있어서, 화면에서 나누면 그 자리를 화면 수만큼 다시 만들게 된다.
 */
class FunnelDtoTest {
    ...
    /**
     * 아래 칸이 위 칸보다 클 수는 없어야 정상이다. 그런데 수집 시작일이 칸마다 다르거나 링크를 직접 받아 들어오면 그런 값이 나온다. 그때 <b>100% 로 잘라 숨기지
     * 않는다</b> — 억지로 맞추면 "이상하다" 는 신호까지 지워져, 보는 사람이 데이터를 의심할 기회를 잃는다.
     */
    @Test
