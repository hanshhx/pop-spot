 ...
 *
 * <p><b>이 기능의 실패 방식은 둘이고, 뒤쪽이 훨씬 나쁘다.</b> 못 고치면 이름 하나가 어색하게 남을 뿐이지만, 잘못 고치면 <b>멀쩡한 일본어 이름을
 * 망가뜨린다</b>. 그래서 "고치는" 시험만큼 "건드리지 않는" 시험을 함께 둔다.
 *
...
class KanaInNameTest {
    ...
    void singleKanaIsFixed() throws Exception {
        assertThat(fix("AK 플라ザ 귀멸의 칼날 팝업스토어")).isEqualTo("AK 플라자 귀멸의 칼날 팝업스토어");
    }

    /** 한글이 하나도 없으면 일본 브랜드 이름일 수 있다. 그건 오염이 아니라 정상이므로 손대지 않는다 — 여기서 건드리면 멀쩡한 이름을 망가뜨린다. */
    @Test
    ...
    /** 표에 없는 글자는 <b>지우지 않고 그대로 둔다.</b> 조용히 없애면 이름이 더 망가지고, 무엇이 잘못됐는지도 안 보인다. 대신 로그에 경고가 남는다. */
    @Test
