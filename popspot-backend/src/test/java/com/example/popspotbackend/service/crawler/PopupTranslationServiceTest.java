 ...
 *
 * <p>실제 이름 60건으로 미리 재 봤을 때, 남은 실패는 대부분 확신 없이 지어낸 이름이었다. 그중에는
 * 관광객을 다른 장소로 보내는 것도 있었다. 그래서 이 서비스의 값어치는 번역 자체보다 <b>못 믿을 값을
 * 버리는 데</b> 있다 — 버리면 화면에 한국어 원문이 나올 뿐이고, 잘못 채우면 사람이 헛걸음한다.
 */
