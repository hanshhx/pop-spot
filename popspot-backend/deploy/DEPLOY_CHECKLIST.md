...
- [ ] 최초 기동은 `POPSPOT_SCHEDULING_ENABLED=false` 로 한다.
      DB 가 아직 진짜가 아닌 구간(빈 DB 헬스체크·임시 공개 DB)에서 만료처리·중복정리가
      DB 를 고치거나 백업 스케줄러가 빈 DB 를 뜨는 것을 막는다.
- [ ] 기동 로그에 `[Scheduling] DISABLED` 가 찍히는지 확인한다.
- [ ] 이 구간의 백업은 앱 스케줄러가 아니라 **수동 `pg_dump` · systemd timer · OCI 볼륨 백업**
      중 하나로 한다. 앱 백업 스케줄러는 이 스위치에 함께 묶여 있다.
- [ ] 쓰기 기능을 연 뒤 **크롤러를 수동으로 한 번** 돌려 결과를 검수하고 백업을 확인한다.
- [ ] 그다음에 `POPSPOT_SCHEDULING_ENABLED=true` 로 바꾸고 **재시작**한다.
- [ ] 재시작 후 `[Scheduling] ENABLED` 와 예약 작업 등록을 확인한다.

> 켜는 것을 잊으면 백업·만료처리·알림·크롤러가 영구히 멈춘 채로 남는다. 조용히 멈추므로
> 화면만 봐서는 알 수 없다.
