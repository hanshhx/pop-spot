...
## 29.20 v2.21-S17 — Spotify 진행바 멈춤 현상 수정

**증상:** Premium 으로 풀트랙 재생 중 진행바가 멈춰 보이거나 실제 위치와 어긋남.

**원인:** Spotify Web Playback SDK 의 `player_state_changed` 이벤트는 play / pause /
seek / 트랙 변경 같은 "상태 전환" 시점에만 발화한다. 재생이 그냥 흘러가는 동안에는
`position` 을 자동으로 갱신해 주지 않아, 마지막 이벤트 시점의 position 에 진행바가
얼어붙은 것처럼 보였다.

**해결 (`useSpotifyPlayer.ts`):**

- 500ms 마다 `player.getCurrentState()` 를 폴링해 `position` / `duration` 을 직접 읽어
  `currentSec` / `durationSec` / `progress` 를 보정.
- `enabled && isReady` 일 때만 인터벌 등록, cleanup 에서 `clearInterval` — 엔진 전환 /
  언마운트 시 누수 없음.
- preview / youtube 엔진은 각자 `timeupdate` / 폴링이 이미 있어 영향 없음.

---

## 29.21 v2.21-S18 — 운영 중 발견된 3종 버그 수정 (검색 UI / Spotify 콜백 / preview 무음)

사용자 제보 3건을 한 번에 정리. 프론트 빌드 통과(42/42), 백엔드 `spotlessApply` 통과.

### 1) 헤더 통합검색 — 부자연스러운 펼침 + 깨진 박스 + 입력창의 "선"

**증상:** ① 펼칠 때 효과가 툭 끊김, ② "통합검색" 옆에 깨진 네모(□) 보임, ③ 검색
입력 시 오른쪽에 세로 선/X 같은 게 나타남.

**원인 (`InlineGlobalSearch.tsx`):**

- ② 칩에 `<kbd>⌘K</kbd>` 단축키 표시가 있었는데, 사용 폰트에 ⌘(U+2318) 글리프가 없어
  `□` 두부(tofu) 로 렌더됨.
- ③ `<input type="search">` 의 브라우저 기본 "검색어 지우기" 컨트롤(`::-webkit-search-
  cancel-button`) 이 입력 중에 노출 — 사용자가 본 "선".
- ① 펼침 폼에 `initial={{ width: 40 }} animate={{ width: "100%" }}` 너비 애니메이션을
  걸었는데, 동시에 `min-w-[220px]` 가 있어 CSS 최소 너비와 framer-motion 너비 보간이
  충돌 → 첫 프레임에 220px 로 튀고 나서 100% 로 가는 끊김.

**해결:**

- ⌘K `<kbd>` 제거, 칩 패딩 `px-4` 로 정리.
- `type="search"` → `type="text"` (`inputMode="search"` + `autoComplete="off"` 유지) —
  취소 버튼 글리프 자체가 사라짐.
- 너비 애니메이션 제거 → `opacity` + `scale(0.97→1)` 페이드, `transformOrigin: right
  center`, 고정 너비 `w-[220px] md:w-[320px]`. min-w 충돌 소거로 매끄럽게 펼쳐짐.

### 2) Spotify 연결 후 음악 페이지가 아니라 인트로로 튕김

**증상:** Spotify 연결(OAuth)을 마치고 돌아오면 음악 탭이 아니라 인트로 페이지로 감.
연결 완료 토스트도 안 뜸.

**원인 두 가지:**

- **이중 `?`** — `SpotifyAuthController` 가 `frontendRedirect + "?spotify=connected"` 로
  URL 을 만들었는데, 환경변수 `SPOTIFY_OAUTH_FRONTEND_REDIRECT` 에 이미 쿼리가 있으면
  (`https://popspot.co.kr/?tab=MUSIC`) → `...?tab=MUSIC?spotify=connected` 처럼 `?` 가
  두 개. 깨진 URL.
- **middleware 미인식** — `/?spotify=connected` 가 deep-link 목록(`tab/popup/...`)에
  없어, 루트 진입 미들웨어가 "그냥 루트 접근" 으로 보고 `/intro` 로 redirect.
- **토스트 안 뜸** — `SpotifyConnectButton`(콜백 결과 토스트 담당)이 음악 탭 안에만
  마운트되는데, 연결 후 다른 탭/인트로로 가면 그 컴포넌트가 없어 토스트가 안 뜸.

**해결:**

- 백엔드 `frontendRedirectWith(status)` 헬퍼 신설 — frontendRedirect 의 기존 쿼리를
  잘라내고 항상 `?tab=MUSIC&spotify=<status>` 만 붙임. 이중 `?` 원천 차단 + 음악 탭으로
  확실히 복귀. (denied / connected / error 세 경로 모두 적용.)
- `middleware.ts` 의 `DEEP_LINK_PARAMS` 에 `"spotify"` 추가 — 콜백 복귀 URL 이 인트로로
  안 튕기고 `entered=1` 붙어 메인 통과.

### 3) Spotify 미연동 시 "30초 미리듣기" 가 아예 재생 안 됨 (무음)

**증상:** 프리미엄 미연동(무료/비연결) 사용자가 곡을 눌러도 소리가 전혀 안 남.
"30초 미리듣기" 라고 했는데 무음.

**원인 (`usePreviewPlayer.ts`):** 브라우저 자동재생 정책. 곡 클릭 → 백엔드 매칭(비동기
fetch) → `setCurrent(enriched)` → 그제서야 preview 엔진이 `audio.play()` 호출. 이 시점은
이미 네트워크 콜백 안이라 "사용자 제스처 창" 을 벗어나 브라우저가 소리 있는 자동재생을
차단 → `.catch(() => {})` 가 조용히 삼켜 무음. 게다가 트랙마다 `new Audio()` 를 새로 만들어
한 번 unlock 된 엘리먼트를 재활용하지 못함.

**해결:**

- audio 엘리먼트를 트랙마다 새로 만들지 않고 **1개를 재사용** — 한 번 재생되면 그
  엘리먼트는 세션 내내 unlock 상태라 이후 트랙은 정상 자동재생.
- 자동재생이 거부되면 `document` 에 **1회용 `pointerdown` 리스너**를 걸어 다음 클릭
  (아무 곳이나, 보통 ▶ 버튼)에 `play()` 재시도 → 첫 곡도 클릭 한 번으로 살아남.
- `wantPlayRef` / `retryPendingRef` 로 재생 의도 추적 + 리스너 중복 등록 방지.
- preview / spotify 엔진의 `previewUrl` 자체는 백엔드 iTunes 폴백(S15)이 채워주므로,
  **백엔드 재배포가 선행되어야** preview 가 채워진 트랙이 늘어남.

### 배포 메모

- **프론트**: middleware / InlineGlobalSearch / usePreviewPlayer — Vercel 자동 배포.
- **백엔드**: `SpotifyAuthController` 1파일 — JAR 재빌드 + 재배포 필요. (S15 iTunes 폴백
  + S16 한영 검색이 아직 운영에 없으면 이번에 함께 올라감.)
...
## 29.35 v2.30.2 — 데스크톱 네비 텍스트형 재설계

v2.30.1(캡슐 네비) 후 "데스크톱 네비가 하단 독을 그대로 위로 올린 느낌"이라는 피드백(레퍼런스: 상단 텍스트 GNB). 데스크톱만 텍스트형으로 재설계, 모바일 독은 유지.

- **Header 데스크톱 네비**: 아이콘(`item.icon`) + 캡슐 컨테이너 제거 → **텍스트 메뉴만**(지도·코스·음악·여권·MY·동행), `gap-7` 간격.
- 액티브: `text-foreground font-bold` + **라임 언더라인**(`after:h-[3px] after:bg-lime-400`). 비활성 `text-muted-foreground font-medium hover:text-foreground`.
- 캡슐/배경 제거로 헤더 바에 자연스럽게 통합(일반 웹사이트 상단 GNB 패턴). 아이콘은 모바일 독에서만 사용.

### 검증 (미리보기 실측)

- 데스크톱(1280): 네비 `svg 아이콘 0개`, 라벨 6개 텍스트("지도"~"동행"), 액티브 `::after backgroundColor=rgb(168,230,69)`(lime-400) · `fontWeight=700`.

> 빌드 통과 · TS OK. 프론트 전용 → Vercel 자동 배포.

---

## 29.36 v2.30.3 — 네비게이션 크기 확대

"네비가 너무 작다" 피드백. 모바일 독 + 데스크톱 네비 동시 확대.

- **모바일 독(`BottomDock`)**: 버튼 `h-14 md:h-16`(56/64) → **`h-16 md:h-[72px]`(64/72)**, 아이콘 `size-6→7`, 라벨 `11→12px`, 컨테이너 `p-2→2.5`, 바 폭 `max-w-520→560`, 라운드 `1.25→1.375rem`.
- **데스크톱 네비(`Header`)**: 텍스트 `15→17px`, 간격 `gap-7→8`, `py-1→1.5`.

### 검증 (미리보기 실측)

- 모바일(375): 독 바 높이 86px, 버튼 64px, 아이콘 ~31px, 라벨 12px, `scrollWidth==375`(오버플로우 0).
- 데스크톱(1280): 네비 `fontSize=17px`, `display=flex`.

> 빌드 통과 · TS OK. 프론트 전용 → Vercel 자동 배포.

---

## 29.37 v2.31 — 게스트·방문자 통계 (Vercel Analytics + 어드민 방문 통계)

"게스트(비로그인)가 왔다 간 걸 어드민에서 보고 싶다" 요청. 게스트는 DB 유저 레코드가 없어 기존엔 추적 불가 → **익명 방문 로그** 신설 + Vercel Analytics 병행.

### 프론트

- `@vercel/analytics` 추가, `layout.tsx` 에 `<Analytics/>`(쿠키리스·익명 방문 지표는 Vercel 대시보드).
- `components/VisitTracker.tsx`(신규) — 앱 로드/경로 변경 시 익명 비콘 `POST /api/visits { visitorId, path, guest }`. **IP·개인정보 미전송.** `visitorId` = localStorage 랜덤 UUID(개인 식별 불가). 세션당 경로별 1회(sessionStorage 가드), `fetch keepalive`, 실패 조용히 무시.

### 백엔드 (익명, IP 미저장)

- `VisitLog` 엔티티 + Flyway `V15__visit_log.sql`(visitor_id·path·guest·created_at + 인덱스 2개).
- `VisitLogRepository` — 오늘/7일 고유 방문자, 페이지뷰, 게스트/회원별, 일별 시계열, 인기 경로 native 집계.
- `VisitService.record()`(길이 절단) + `getStats()`. `VisitStatsDto`(record).
- `VisitController` `POST /api/visits`(공개 — 게스트도 기록, 항상 204). `AdminVisitController` `GET /api/admin/visits/stats`(ADMIN 가드).

### 어드민

- `/admin` 에 **"방문 통계"** 탭(VISITS) — 오늘 방문자·페이지뷰·게스트·회원·7일 방문자 5카드 + 최근 7일 막대그래프 + 인기 페이지 Top 8.

### 개인정보

- 방침 제2조에 도구 명시(Vercel Web Analytics + 자체 익명 방문 로그, IP·개인정보 미수집). 기존 "개인 식별 불가능 통계"(제2조)·"접속 로그 3개월"(제3조) 조항과 일관.

### 검증

- **프론트 빌드 118/118 통과.** 백엔드는 로컬 Gradle loopback 이슈로 미컴파일 — AOSP 수동 포맷. **배포 전 `./gradlew spotlessApply build -x test` + 첫 부팅 V15 마이그레이션** 필요.

> 프론트 빌드 통과. 백엔드 배포 필요. Vercel Analytics 는 프로젝트 설정에서 활성화 필요.

---

## 29.38 v2.32 — 메인 페이지 SEO 강화 (서버 렌더 본문 + 키워드 우선 타이틀)

"메인 `https://popspot.co.kr/` 이 더 많이 노출됐으면 좋겠다, 노출 자체가 별로다" 요청. GSC 실측으로 원인을 특정하고 구조를 고쳤다.

### 진단 (GSC 실측)

- 3개월 누적 메인 `/` 노출 **≈5회**. 반면 슬라이스 랜딩은 잠실 43·성수 39 등 정상 노출.
- 원인: 메인이 `"use client"` + `useSearchParams()` 최상위라 **본문 전체가 SSR 되지 않음** — 서버 HTML 엔 스피너만. 구글은 렌더 큐를 거쳐야 내용을 보므로 메인을 사실상 "브랜드어(팝스팟)"로만 색인. "서울 팝업스토어" 같은 헤드 키워드로 잡히지 않음.

### 조치

- `app/page.tsx`(클라이언트 앱) → **`HomeClient.tsx`** 로 `git mv`(내용 무변경).
- 새 `app/page.tsx` = **서버 컴포넌트 래퍼**:
  - `metadata` 브랜드 우선 → **키워드 우선** 타이틀 "서울 팝업스토어 지도 · 오늘 여는 팝업 한눈에" + description + `canonical: https://popspot.co.kr`.
  - **서버 렌더 `sr-only` SEO 블록** — H1 "서울 팝업스토어 지도", 서비스 설명, **지역/카테고리 슬라이스 내부 링크**(`REGIONS`·`CATEGORIES` 맵). 크롤러는 읽고 사용자 화면엔 변화 없음.
  - `<Suspense>` 로 `<HomeClient/>` 감쌈 — 앱 동작·UX 그대로.

### 검증

- `curl` 로 서버 HTML 확인: 키워드 타이틀·H1·`/popups/seongsu`·`/popups/jamsil` 내부 링크 포함.
- 미리보기 실측: 앱 정상 마운트(헤더·독·웰컴 렌더), SEO H1 DOM 존재.
- 프론트 빌드 **118/118 통과.** 프론트 전용 → Vercel 자동 배포.

### 배포 후 필수 (구글 재크롤 촉진)

- GSC → **URL 검사**에 `https://popspot.co.kr/` → **색인 요청**.
- **sitemap 재제출**. 재크롤·재평가는 즉시 아님(보통 며칠~2주). "내일 바로 급상승"은 비현실적 — 구조는 고쳤고 이제 구글 재색인 대기.

> 프론트 빌드 통과 · Vercel 자동 배포. 배포 후 GSC 색인 요청 + sitemap 재제출 필요.

---

## 29.39 v2.33 — 자동수집 다건 추출 (하루 8건 병목 해소)

"데이터 수집이 하루 8건뿐 → 총 90~120개에서 정체. 1000건 이상 원함. 정책 위반 없이 해결" 요청. 코드에서 병목을 특정하고 구조를 고쳤다.

### 진단 (병목 2개)

크롤러는 키워드당 **네이버 블로그 30 + 뉴스 30 + 카카오 웹 30 + 블로그 30 = 최대 120 snippet** 을 실제로 가져온다. 그런데:

- **❶ `PopupNormalizationService.MAX_SNIPPETS_PER_REQUEST = 8`** — 가져온 120개 중 **앞 8개만 LLM 에 투입**, 112개(93%)는 쿼터를 이미 쓴 뒤 그냥 버림.
- **❷ 프롬프트 "동일 팝업 1개에 대한 JSON 1개를 출력해"** — 그 8개에 서로 다른 팝업이 섞여 있어도 **1개로 뭉갬**.

결과: 85키워드 × 1 = 하루 최대 85 → 중복(externalId)·신뢰도 0.8 미달 제거 후 **신규 ≈8건/일**. 증상과 일치.

### 조치 (A안 — 추출 로직만, 추가 스크래핑 0)

- **`normalize()` → `normalizeAll()`** : LLM 한 번 호출로 snippet 묶음 안의 서로 다른 팝업을 **모두** `List<NormalizedPopup>` 로 추출. 프롬프트를 JSON **배열** 출력으로 교체, 각 원소에 `sourceIndex`(근거 snippet 번호) 추가.
- **`MAX_SNIPPETS_PER_REQUEST` 8 → 40** : 이미 가져와서 버리던 데이터를 더 활용. API 호출 수·쿼터·**LLM 호출 수(키워드당 1회)는 그대로** — Groq RPM 추가 부담 0.
- **소스 교차 배치(interleave)** : `fetchSnippetsForKeyword` 가 4채널을 순차 concat → 라운드로빈으로 바꿈. 앞 40개가 네이버 블로그에만 쏠리던 편향 제거 + `sourceIndex` 매핑 순서 고정.
- **출처 매핑** : `pickPrimarySource(snippets, sourceIndex)` 로 팝업별 근거 snippet 을 sourceUrl/sourceName 에 연결(범위 밖·null 이면 첫 snippet 대체).
- **응답 방어** : `extractArray()` 가 배열/`{"popups":[...]}` 래핑/객체 1개 모두 수용.
- **통계 분리** : `llmCalls`(호출 수) vs `normalized`(후보 수) 별도 카운트.
- `NormalizedPopup` 에 `sourceIndex`(임시 힌트, DB 미저장) 필드 추가.

### 유지 (품질·정책 가드)

- 신뢰도 **0.8 게이트**·이름/서울 후처리 검증·중복제거(externalId = SHA-256(name|location|startDate))·PII 규칙(개인 연락처·닉네임 제외) 그대로.
- **공식 네이버·카카오 검색 오픈API**(title/description/link 만, 본문 스크래핑 X) — 저작권법 제35조의5 공정이용·출처표시 유지. 일일 한도(네이버 25,000·카카오 30,000) 안. **§10-2(수집 팝업 sitemap 미색인)는 SEO 정책이라 수집량과 무관 — 무영향.**

### 기대치 (솔직)

- 하루 ~8 → **~50-100건+** (키워드당 다건 추출). "1000 동시 라이브" 는 서울에 그만한 팝업이 동시에 없어 비현실적 — 1000은 진행+지난 아카이브(B안) 필요, 이번엔 미포함.
- 검증: 첫 크롤 로그의 `autoPublished`/`normalized`/`llmCalls` 로 확인.

### 검증

- 로컬 Gradle loopback 이슈로 미컴파일 — AOSP 수동 포맷. **배포 전 `./gradlew spotlessApply build -x test` 필수.** 호출부(Orchestrator) 1곳만 변경, 테스트 영향 없음.

> 백엔드 변경 — 배포 필요(`./gradlew spotlessApply build -x test` → scp → 재시작). DB 마이그레이션 없음(스키마 무변경).

---

## 29.40 v2.33.1 — 다건 추출 정확도 가드

v2.33 로 키워드당 팝업을 여러 개 뽑으면서 생긴 부작용을 프롬프트에서 방어. "정확도 80%(0.8)면 그대로 아니냐"는 지적에 대한 보강.

### 배경 (다건 추출의 정확도 리스크)

- 예전엔 snippet 8개가 팝업 1개를 집중 뒷받침 → 쉽게 0.8 통과. 다건 추출은 40개가 여러 팝업에 쪼개져 **팝업당 근거가 1~2개** → confidence 가 낮게 나와 더 많이 걸러진다(품질 바는 유지, 오히려 빡세짐).
- 새 리스크: 한 프롬프트에 여러 팝업이 섞여 **교차오염**(A 팝업 날짜를 B 에 붙임)·**유사중복**(같은 팝업을 이름만 다르게 2번)이 생길 수 있다. 완전 일치 중복은 externalId 로 잡지만 유사중복은 못 잡는다.

### 조치 (프롬프트 규칙 2개 추가, 코드/스키마 무변경)

- 규칙 4 강화: "팝업 이름·존재가 **확실하지 않으면 넣지 마 — 물량보다 정확도 우선**(애매하면 버려)."
- 규칙 7 신설: "각 팝업의 날짜·장소·카테고리는 그 팝업을 다룬 snippet 에서만 가져와 — **한 팝업 정보를 다른 팝업에 섞지 마(교차오염 금지)**. 같은 팝업이 이름만 조금 다르게 여러 번 나오면 **하나로 합쳐**(유사중복 병합)."

### 유지

- 신뢰도 **0.8 게이트** 그대로(정확도 바를 낮추지 않음). 후속 옵션(문턱 0.85 상향 / 게시 전 2차 검증 LLM)은 첫 크롤 결과를 보고 판단.

> 백엔드 — v2.33 과 같은 jar 로 배포. 스키마 무변경.
